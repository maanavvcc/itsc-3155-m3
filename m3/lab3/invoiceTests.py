...
def test_invoice_add_item_appends_item() -> tuple[bool, str]:
    ...
    invoice = Invoice("invoice")  # Replace None with the correct code
    item = Item("item",1,1.0,.2)
    invoice.add_item("item",1,1.0,.2)
    fail_case = (item.calculate_item_total_price() != invoice.items[0].calculate_item_total_price())  # Replace True with the correct code
...
def test_invoice_add_item_checks_discounted() -> tuple[bool, str]:
    ...
    invoice = invoice = Invoice("invoice")  # Replace None with the correct code
    invoice.discounted = True
    item = Item("item",1,1.0,.2)
    invoice.add_item("item",1,1.0,.2)
    fail_case = (item.discount_percent != invoice.items[0].discount_percent)  # Replace True with the correct code
